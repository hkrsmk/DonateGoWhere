...
def uncurl(file_to_edit):
    '''remove curl commands and empty lines from the appended curl output, overwriting the final file each time
    ...
    with open(file_to_edit+'.json') as oldfile, open(file_to_edit+'new.json', 'w+') as newfile:
